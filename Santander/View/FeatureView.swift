...
//  Santander
//
//  Created by Guilherme Garcia on 31/07/23.
//
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHGrid(rows: [GridItem()]) {
                ForEach(viewModel.features, id: \.id) { feature in Rectangle()
                        .foregroundColor(.clear)
                        .frame(width: 110, height: 96)
                        .background(.white)
                        .cornerRadius(6)
                        .shadow(color: .black.opacity(0.15),
                                radius: 4.5, x: .zero, y: 4)
                        .overlay {
                            VStack {
                                AsyncImage(from: URL(string:
                                              feature.iconUrl), frame: CGSize(width: 30, height: 30))
                                { image in
                                    image
